...
function HorizonPlot(svg, w, h, timeseries, levels, deployed)
{
	this.w = w;
	this.h = h;
	...
	this.group.selectAll('g.levels').data(levelData).enter().append("g")
		.attr('class', 'levels')
		.style("fill", function(d) { return HORIZON_COLORS[0]; })
		.attr("clip-path", function(d, i) { return "url(#" + "horizonPlot_" + d.plotID + ")"; })
		.attr('transform', function(d, i) {
			return 'translate(' + 0 + ',' + -(d.level * d.plotH) + ')';
		});
...
{
	var selection = this.group.selectAll('g.levels').transition().duration(1000)
	if (color) {
		selection.style('fill', color)
	}
	else
	{
		selection.style('fill', function(d) { return HORIZON_COLORS[d.level]; })
	}
}
