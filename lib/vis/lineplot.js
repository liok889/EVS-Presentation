...
function LinePlot(group, w, h, options)
{
	...
	// clip rectangle
	var svg = d3.select(getSVG(group.node()));
	...
	var clipPath = defs.append("clipPath")
		.attr("id", "linePlot_" + this.plotID);
		
	this.clipRect = clipPath.append("rect")
		.attr("x", 0).attr("y", 0)
		.attr("width", this.w-options.padW*2).attr("height", this.h-options.padW*2);
		
	// data group
	this.dataGroup = this.group.append("g")
		.attr('class', 'lineplotData')
		.attr('clip-path', 'url(#' + 'linePlot_' + this.plotID + ')')
	...
	// brush group
	this.brushGroup = this.group.append('g')
		.attr('class', 'brush')
		.attr('transform', 'translate(' + options.padW + ',' + options.padH + ')');
	if (options.multiscale)
	{
		(function(plotter) {
			plotter.brush = d3.svg.brush()
				.x(d3.scale.identity().domain([0, plotter.w-2*plotter.options.padW]))
			...
			plotter.brushGroup
				...
				.selectAll('rect')
					.attr('y', 0)
					.attr('height', plotter.h-2*plotter.options.padH);
		})(this);
	}
...
LinePlot.prototype.brushUpdate = function()
{
	var timeRange = this.getTimeRange();
	var brushScale = d3.scale.linear()
		.domain([0, this.w-2*this.options.padW])
	...
	else
	{
		...
		if (!this.dependent) 
		{
			// add a new group for the dependent chart
			var g = this.group.append('g')
				.attr('transform', 'translate(0,'+(this.h+5) + ')');

			this.dependent = new LinePlot(g, this.w, this.h, copyOptions(this.options));
			this.dependent.parent = this;

			// add time series
			for (var i=0, N=this.plots.length; i<N; i++) {
				var plot = this.plots[i];
				this.dependent.addTimeseries(plot.timeseries, null, plot.color);
			}
		}
		var dependent = this.dependent;
		dependent.setConstraintTimeRange(brushedTimes);
						
		// set date
		var masterPlot = this;
		while (masterPlot.parent) {
			masterPlot = masterPlot.parent;
		}
		var dateRange = masterPlot.getDateRange();
		if (dateRange) 
		{
			...
			var dependentDate = 
			[
				timeScale.invert(brushedTimes[0]),
				timeScale.invert(brushedTimes[1])
			];

			var timeLen = dependentDate[1]-dependentDate[0];
							
			if (timeLen < 0.3 * 365*24*60*60*1000) {
				dependent.options.tickFormat = '%e %b, %Y';
			}
			else if (timeLen < 3*365*24*60*60*1000) {
				dependent.options.tickFormat = '%b, %Y';
			}
			dependent.setDateRange(dependentDate);
		}
	}
}

LinePlot.prototype.getTimeRange = function()
{
	if (this.constraintTimeRange) {
		return this.constraintTimeRange;
	}
	else
	{
		return [0, 1];
...
LinePlot.prototype.setConstraintTimeRange = function(constraint)
{
	this.constraintTimeRange = constraint;
	this.updateDataTransform();
	if (this.dependent) {
		this.brushUpdate();
	}
...
LinePlot.prototype.getDateRange = function()
{
...
LinePlot.prototype.updateDataTransform = function()
{
	...
	if (this.constraintTimeRange) 
	{
		var timeRange = this.constraintTimeRange;
		transform +=
			'scale(' + (1/(timeRange[1]-timeRange[0])) + ',1) ' +
			'translate(' + (-timeRange[0]) + ',0) ';
	}
...
LinePlot.prototype.remove = function() {
